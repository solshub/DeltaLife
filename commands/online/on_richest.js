module.exports = {
	run: async ({message, conn}) => {
		...
		function getPlayers(resolve, reject) {
			const query = 'SELECT p.name, p.cash, u.user FROM player p, user u ' +
				'WHERE p.user = u.id ORDER BY cash DESC LIMIT 10';
		...
		let embed = {
			...
			author: {
				name: 'Richest characters leaderboard',
				icon_url: process.env.ICO_TROPHY
			},
			description: '\`User, character\'s name and cash:\`',
			timestamp: new Date()
		};

		for(let i = 0; i < result.length; i++) {
			embed.description += `\n**${i + 1}.** <@!${result[i].user}>'s ` +
				`${result[i].name} with $${parseFloat(result[i].cash).toFixed(2)}`; }
